...
def apply_geo_chain(
    ...
    source_text: str,
    methods: list[GEOMethod],
    target: str,
    goal_type: GoalType = GoalType.CONCEPT,
    queries: tuple[str, ...] | None = None,
) -> str:
    """
    Apply a list of GEO methods in sequence (deterministic chain).
    text_0 = source_text, text_{i+1} = GEO(methods[i], text_i).
    Same target and goal_type for each step.
    """
    if not methods:
